...
NAN_METHOD(Socket) {
    ...
    // Use platform-specific socket flags
    ...
    fd = socket(AF_UNIX, type, 0);
    if (fd != -1) {
        // Set non-blocking and close-on-exec flags manually on macOS
...
NAN_METHOD(Close) {
    ...
    // the case in Linux and macOS, then <unistd.h> should add:
    //
    ...
    //
    // But since we actually only care about Linux and macOS, we don't have to worry about
